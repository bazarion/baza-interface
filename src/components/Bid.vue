    ...
    <div class="make-offer">
        <b-card no-body class="box-bid-price mb-3">        
            <b-card-body>            
                ...
                <b-btn class="btn-custom btn-cancel mr-3" @click="$emit('bid-cancel')">
                    Cancel
                </b-btn>
                ...
                <b-button
                    ...
                    @click="bid()"
                    >
                    Offer Now <b-icon v-if="!isFTApproved" icon="info-circle-fill" v-b-tooltip.hover.top="`You need to approve the market to use your fungible token ${ftSymbol}`"></b-icon>
                </b-button>
                </div>
...
export default {
    ...
    created: async function () {
        ...
        let owner = await this.market.getOwnerByTokenId(this.tokenId)
        ...
        let isBidded = (!(await this.market.getBidShare(this.tokenId, this.address)).eq(0))
        this.isDisabled = (owner === this.address.toString())
        this.isDisabled = this.isDisabled || isBidded
    ...
    methods: {
        ...
        async bid () {
            try {
                ...
                let tx = await this.market.bid(this.tokenId, price)
                this.$bvToast.show('tx-confirm')
                await tx.wait()
                this.$bvToast.show('tx-success')
                this.$bvToast.hide('tx-confirm')
